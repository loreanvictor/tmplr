...
test('applies pipes properly.', async () => {
  ...
  const res7 = await evaluate(store, 'Hello {{ vars.name | skip: jo | trim: x }}!!')
  const res8 = await evaluate(store, 'Hello {{ vars.name | skip: lu | trim: oe }}!!')
  const res9 = await evaluate(store, 'Hello {{ vars.name | weird }}!!')
